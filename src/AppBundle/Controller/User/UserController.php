	...
	{	
		...
		if($form->isValid()){
			...
			if($data['club'] != null){
				...
				if($dql == ''){
					$dql .= "Select u.id, u.name, u.lastName, g.name as gender, c.name as country, u.playingLevel from AppBundle:User u, AppBundle:Country c, AppBundle:Gender g, AppBundle:Club cl where cl.id = u.club and u.country = c.code and g.code = u.gender and cl.clubName = :club";
				}else{
			...
			$sq = $query->getSql();
			$pa = $query->getParameters();
			$players = $query->getResult();

			return $this->render('user/searched_players_list.html.twig', array('pa' => $pa, 'sq' => $sq, 'form' => $form->createView(), 'data' => $data, 'query' => $query, 'players' => $players));
		}
